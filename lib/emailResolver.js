...
export async function resolveTeachers(teachers, courseName, accessToken, userCampus, taSheetRows = []) {
  ...
  // Discard GCR teachers that couldn't be resolved at all — if they don't appear
  // in the TA/LD sheet AND have no resolvable email, they add no useful info.
  const resolvedFiltered = resolvedFromGCR.filter((p) => p.source !== 'notFound');

  return [...resolvedFiltered, ...sheetOnlyTAs];
}

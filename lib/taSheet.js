 ...
 *           *coverage score* = number of distinct GCR teachers that find a
 *           match (>= threshold) in the group's instructor list.
 *           Tiebreaker: max single-pair similarity.
 *           Example: AI Lab (matches Ali Umer + Zill-E-Huma = 2) beats
 *           DB Systems Lab (matches only Ali Umer = 1) even though both
 *           have max similarity 1.0.
 ...
 *           This separates multi-batch offerings of the same course code.
 *
...
export function matchCourseInSheet(gcrTeacherNames, sheetRows) {
  ...
  let bestCoverage = 0;  // primary:   # of distinct GCR teachers matched
  let bestMaxSim   = 0;  // secondary: max single-pair similarity (tiebreaker)
  let bestGroup    = [];

  for (const [, groupRows] of groups) {
    const groupInstructors = [...new Set(groupRows.map((r) => r.courseInstructor).filter(Boolean))];

    // Coverage: count how many distinct GCR teachers find a match >= threshold
    // in this group's instructor list
    let coverage = 0;
    let groupMaxSim = 0;
    for (const gcrName of gcrTeacherNames) {
      const best = maxSimilarity(groupInstructors, [gcrName]);
      if (best >= CONFIDENCE_THRESHOLD) coverage++;
      if (best > groupMaxSim) groupMaxSim = best;
    }

    // Primary sort: coverage count; secondary: max pairwise similarity
    if (
      coverage > bestCoverage ||
      (coverage === bestCoverage && groupMaxSim > bestMaxSim)
    ) {
      bestCoverage = coverage;
      bestMaxSim   = groupMaxSim;
      bestGroup    = groupRows;
    }
  }
